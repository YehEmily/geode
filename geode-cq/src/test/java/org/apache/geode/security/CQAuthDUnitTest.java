...
import static org.apache.geode.security.SecurityTestUtil.assertAuthorized;
...
public class CQAuthDUnitTest {
  ...
  // TODO: startupRule.startClient?

  @Rule
  public ServerStarterRule server =
      new ServerStarterRule().withProperty(SECURITY_MANAGER, SimpleTestSecurityManager.class.getName())
                    .withRegion(RegionShortcut.REPLICATE, REGION_NAME);

  @Test
  public void testCQExecuteNotAuthorized() {
  ...
  @Test
  public void testCQExecuteAuthorized() {
    String query = "select * from /AuthRegion";
    int serverPort = server.getPort();

    client1.invoke(() -> {
      ClientCache cache = createClientCache("DataReadAuthRegion", "DataReadAuthRegion", serverPort);
      final Region region = createProxyRegion(cache, REGION_NAME);
      Pool pool = PoolManager.find(region);
      QueryService qs = pool.getQueryService();
      CqAttributes cqa = new CqAttributesFactory().create();
      ...
      assertAuthorized(cqResults.asList());
    });
  }
}
